...
@interface MTMainScene()
@property(nonatomic, strong)NSMutableArray *mapArray;
@property(nonatomic, strong)MTTileMap *curtitleMap;
...
- (void)initBackground {
    ...
    //    [backNode setSize:CGSizeMake(KSCREEN_WIDTH, KSCREEN_HEIGHT)];
...
- (void)initHeroInfo {
    ...
    int top = KSCREEN_HEIGHT-15*_mapScale;
    ...
    [_labelNode_mapindex setText:[NSString stringWithFormat:@"---------%ld层----------", (long)_curMapIndex]];
    ...
    [_labelNode_hp setText:[NSString stringWithFormat:@"生命值: %ld", (long)_hero.HeroHP]];
    ...
    [_labelNode_att setText:[NSString stringWithFormat:@"攻击力: %ld", (long)_hero.Attack]];
    ...
    [_labelNode_def setText:[NSString stringWithFormat:@"防御力: %ld", (long)_hero.Defense]];
    ...
    [_labelNode_gold setText:[NSString stringWithFormat:@"金币数: %ld", (long)_hero.Gold]];
    ...
    [_labelNode_exp setText:[NSString stringWithFormat:@"经验值: %ld", (long)_hero.Experience]];
    ...
    if (_mapArray.count > 1) {
        MTTileMap *map = [_mapArray objectAtIndex:1];
        ...
        _predictSpriteNode.position = CGPointMake(140*_mapScale, KSCREEN_HEIGHT-50);
    ...
    [_labelNode_rkey setText:[NSString stringWithFormat:@"%ld", (long)_hero.RedKeyCount]];
    ...
    [_labelNode_bkey setText:[NSString stringWithFormat:@"%ld", (long)_hero.BlueKeyCount]];
    ...
    [_labelNode_ykey setText:[NSString stringWithFormat:@"%ld", (long)_hero.YellowKeyCount]];
...
- (void)initHero
{
    _hero = [[MTHeroSprite alloc] initWithPosition:[self getPointWithIndexPoint:_curtitleMap.down] withScale:_mapScale];
...
- (void)initMap
{
    ...
    for (int i=0; i<=21; i++) {
        MTTileMap *map = [[MTTileMap alloc] initWithIndex:i];
...
- (void)moveMapWithNeedUpdate:(BOOL)isNeedUpdate {
    ...
    if (_hero.currectPoint.y > 4 && isNeedUpdate) {
        _offsetTop = -1;
        _curtitleMap.position = CGPointMake(_curtitleMap.position.x, KSCREEN_HEIGHT-_mapSize.height);
        ...
        _offsetTop = -1;
        [_curtitleMap runAction:[SKAction moveToY:KSCREEN_HEIGHT-_mapSize.height duration:0.15]];
...
- (void)updateMap:(BOOL)isUpMap {
    ...
    _curtitleMap.position = CGPointMake(KSCREEN_WIDTH-_mapSize.width*_mapScale-32, 0);
...
- (BOOL)talkOtherWithGid:(int)gid {
    ...
    if (value && [value integerValue] > 0) {
        NSLog(@"门砸%ld", (long)[value integerValue]);
        return YES;
    }
...
- (void)talkToNPCWithGid:(int)gid point:(CGPoint)point{
    ...
    if (value) {
        ...
        _npcSprite = [[MTNPCSprite alloc] initWithType:npcType];
...
- (BOOL)fightEnemyWithGid:(int)gid position:(CGPoint)pos{
    ...
    NSString *value = [props valueForKey:@"enemy"];
    int type = [value intValue];
    MTEnemySprite *enemy = [MTEnemySprite initWithType:type];
...
- (void)showFightLostHPTip:(NSInteger)lostHP position:(CGPoint)pos{
    ...
    [node setFontSize:14];
    [node setText:[NSString stringWithFormat:@"-%ld", (long)lostHP]];
...
- (void)getItemWithGid:(int)gid {
    ...
    if (value && [value integerValue] > 0) {
        ...
        [ProgressHUD showSuccess:[NSString stringWithFormat:@"攻击力 +%ld ", (long)[value integerValue]]];
    }
    // 防御力
    value = [props valueForKey:@"Defense"];
    if (value && [value integerValue] > 0) {
        _hero.Defense += [value integerValue];
        [ProgressHUD showSuccess:[NSString stringWithFormat:@"防御力 +%ld ", (long)[value integerValue]]];
    }
    // 血
    value = [props valueForKey:@"HP"];
    if (value && [value integerValue] > 0) {
        _hero.HeroHP += [value integerValue];
        [ProgressHUD showSuccess:[NSString stringWithFormat:@"生命值 +%ld ", (long)[value integerValue]]];
    }
    ...
    if (value && [value integerValue] > 0) {
        ...
        [ProgressHUD showSuccess:[NSString stringWithFormat:@"经验 +%ld", [value integerValue]*100]];
    }
    // 金币
    value = [props valueForKey:@"coin"];
    if (value && [value integerValue] > 0) {
        _hero.Gold += [value integerValue];
        [ProgressHUD showSuccess:[NSString stringWithFormat:@"金币 +%ld", (long)[value integerValue]]];
    }
...
- (void)removeTileWithMapIndex:(NSInteger)mapIndex withType:(enumMapLayerType)type WithPoint:(CGPoint)point {
    ...
    MTTileMap *map = [_mapArray objectAtIndex:mapIndex];
    switch (type) {
        case kMapLayer_Wall:
...
- (void)readGame {
    ...
    if (curMapIndex) {
        ...
        _hero = nil;
        _hero = [[MTHeroSprite alloc] initWithPosition:[self getPointWithIndexPoint:curpoint] withScale:_mapScale];
